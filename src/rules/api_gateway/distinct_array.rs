...
/// Rule: use `distinct()` from `#library/utils/arrays` instead of manual dedup patterns.
    ...
    if node.kind() == "array" {
        if is_spread_new_set(node, source) {
            ...
            violations.push(RuleViolation {
                ...
                fix_suggestion: "use distinct() from #library/utils/arrays instead of [...new Set()]".to_string(),
            });
    ...
    if node.kind() == "call_expression" {
        if is_array_from_new_set(node, source) {
            ...
            violations.push(RuleViolation {
                ...
                score,
                code_sample: line.trim().to_string(),
                fix_suggestion: "use distinct() from #library/utils/arrays instead of Array.from(new Set())".to_string(),
            });
            return;
        }
        ...
        if is_reduce_dedup(node, source) {
            ...
            violations.push(RuleViolation {
                ...
                score,
                code_sample: line.trim().to_string(),
                fix_suggestion: "use distinct() from #library/utils/arrays instead of manual dedup with reduce".to_string(),
            });
            return;
        }
    }
